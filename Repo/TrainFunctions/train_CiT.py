...
from torcheval.metrics import MulticlassAccuracy
import torch
from ConvFormer.CiT import CiT
...
def train_CiT(config, img_h, img_w, patch_size, d_model, tuning_mode = False):
    CIFAR_Train, CIFAR_Val, CIFAR_Test = return_datasets(size=40000, batch_size=12)
    device = get_device()
    model = CiT(img_h = img_h, img_w = img_w, d_model = d_model, patch_size=patch_size, mask_fidelity=config['mask_fidelity'], dropout_p=config['dropout_p']).to(device)
